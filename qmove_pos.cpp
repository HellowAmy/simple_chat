...
QSize qmove_pos::move_xr(QPoint pos, int space)
{
    return move_group(pos,space,-1,1,true);
}
...
QSize qmove_pos::move_yr(QPoint pos, int space)
{
    return move_group(pos,space,1,-1,true);
}

QSize qmove_pos::move_group(QPoint pos,int space,int row, int column,bool is_reverse)
{
    int max_wide = 0;
    int max_high = 0;
    int count_row = 0;
    ...
    auto fn_move = [&](QWidget *it){
        //记录控件占领的最大区域
        it->move(pos_move);  //移动位置
        int now_wide = (pos_move.x() + it->width()) - pos.x();
        int now_high = (pos_move.y() + it->height()) - pos.y();
        ...
        {
            //每次移动保留点，超距则修改保留点位置
            pos_move = QPoint(pos_move.x() + it->width() + space,pos_move.y());
            count_row++;
            if(count_row >= row)
            {
                pos_move.setX(pos.x());
                pos_move.setY(pos_move.y() + it->height() + space);
                count_row = 0;
            }
        }
        else if(column > 0) //column
        {
            pos_move = QPoint(pos_move.x(),pos_move.y() + it->height() + space);
            count_column++;
            if(count_column >= column)
            {
                pos_move.setX(pos_move.x() + it->width() + space);
                pos_move.setY(pos.y());
                count_column = 0;
            }
        }
...
//    std::vector<QWidget *>::iterator it_begin;
//    std::vector<QWidget *>::iterator it_end;
//    QList<QWidget *>::iterator it_begin;
//    QList<QWidget *>::iterator it_end;
    if(is_reverse)
    {
